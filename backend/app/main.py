...
def _startup_index():
    ...
    try:
        archive_service.refresh_index(force=True)
    except Exception:
        # Fail open: generation can still work even if indexing has an issue
        pass
...
def refresh_index():
    summary = archive_service.refresh_index(force=True)
    return {"ok": True, "projects": summary.get("counts", {})}
