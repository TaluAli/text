...
function ArchiveViewer() {
  ...
  const matrixColumnWidth = useCallback(
    (index) =>
      index === 0
        ? yHeaderWidth
        : Math.max(120, Math.floor(Math.max(matrixSize.width - yHeaderWidth, 320) / Math.max(1, xTicks.length))),
    [matrixSize.width, xTicks.length, yHeaderWidth],
  )

  const matrixRowHeight = useCallback(
    (index) =>
      index === 0
        ? 44
        : Math.max(
            140,
            Math.floor(Math.max(matrixSize.width - yHeaderWidth, 320) / Math.max(1, xTicks.length)) * 0.78 + 22,
          ),
    [matrixSize.width, xTicks.length, yHeaderWidth],
  )

  const renderMatrixCell = useCallback(
    ({ columnIndex, rowIndex, style }) => {
      ...
      return (
        <div style={{ ...style, padding: 4 }}>
          <div className="matrix-cell-wrap">{content}</div>
        </div>
      )
    },
